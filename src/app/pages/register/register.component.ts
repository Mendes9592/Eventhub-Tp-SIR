...
export class RegisterComponent {
  ...
  submit() {
    ...
    this.auth.register(this.form).subscribe({
      next:(r)=>{ this.toast.show('Bienvenue '+r.user.prenom+' ! Compte créé avec succès.'); this.router.navigate(['/']); },
      error:()=>{ this.error.set('Erreur lors de la création du compte.'); this.loading.set(false); }
    });
  }
}
